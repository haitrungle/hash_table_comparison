...
#include "hashtable.h"
/*
    HashTable_QP: hash table using quadratic probing
    ...
    The probing sequence here is a bit unconventional: the table size is a
    power of 2, and the quadratic polynomial is p(i) = i(i+1)/2. That is,
    given a hash h(x), the sequence of indices we will try is
        h(x), h(x) + 1, h(x) + 3, h(x) + 6, h(x) + 10, ... mod 2^n
    They are called the triangular numbers. These actually cover the residue 
    class mod 2^n completely, i.e. when i ranges from 0 to 2^n-1, the above 
    sequence is a permutation of [0, 2^n). This is because for different i 
